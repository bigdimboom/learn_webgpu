...
export class FreeLookCam implements CameraInternal {
  ...
  private _pyrSensitivity: number = 0.035; // pitch yaw roll
  private _moveSensitivity: number = 0.02; // move

  constructor(
    fov = glMatrix.toRadian(65),
    aspectRatio = 1,
    near = 0.01,
    far = 5000
  ) {
  ...
  GetBytes(): number {
    return (
      ...
      (this.proj as Float32Array).byteLength
    );
  }
  ...
  FromLookAt(
    ...
    this._pos = eye;
    this._dir = vec3.normalize(
      vec3.create(),
      vec3.sub(vec3.create(), center, eye)
    );
    this._right = vec3.normalize(
      vec3.create(),
      vec3.cross(vec3.create(), this._dir, up)
    );
    this._up = vec3.normalize(
      vec3.create(),
      vec3.cross(vec3.create(), this._right, this._dir)
    );
    this.Update();
  ...
  Yaw(rad: number) {
    ...
    vec3.normalize(
      this._right,
      vec3.cross(this._right, this._dir, vec3.fromValues(0, 1, 0))
    ); // this prevents camera rolling
    vec3.normalize(this._up, vec3.cross(this._up, this._right, this._dir));
  }
  ...
  Update(model: mat4 = mat4.identity(mat4.create())): void {
    mat4.lookAt(
      this.view,
      this._pos,
      vec3.add(vec3.create(), this._pos, this._dir),
      this._up
    );
  }

  ConfigureMovementSensitivity(val: number) {
    this._moveSensitivity = val;
  }

  ConfigurePitchYawSensitivity(val: number) {
    this._pyrSensitivity = val;
  }
}
